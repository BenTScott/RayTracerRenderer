...
void RGBImage::SetPixel(unsigned x, unsigned y, lin_alg::Vector<3> colour)
{
    ...
    image[3 * width * y + 3 * x + 0] = (unsigned char)round(bounded_colour[0] * 255.0);
    image[3 * width * y + 3 * x + 1] = (unsigned char)round(bounded_colour[1] * 255.0);
    image[3 * width * y + 3 * x + 2] = (unsigned char)round(bounded_colour[2] * 255.0);
};
